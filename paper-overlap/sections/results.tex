...
Let $i_1, j_1$ be $\cstart(C_1)$ and $\cend(C_1)$, and $i_2, j_2$ be $\cstart(C_2)$ and $\cend(C_2)$, respectively. By the definition of the overlap operator, we have $i_1 \leq i_2 \leq j_1 \leq j_2$.
The runs are:
...
With $q_n \in F_1$ and $p_m \in F_2$. Using the construction of $\cAoverlap$, we can build a run $\rho_{\overlap}$ over $\Stream$ as follows:
\\
First, the run follows $\rho_1$ using $\Delta_1$ until position $i_2-1$:
\[ \rho_{\overlap}: q_0 \xrightarrow{(P_{i_1}, L_{i_1})} \dots \xrightarrow{(P_{i_2-1}, L_{i_2-1})} q_{k} \]
Then, at position $i_2$, we use an $\epsilon$-transition of the form $\{(q, \epsilon, (q, p_0)) \mid q \in Q_1\}$  to enter the product state:
\[ \rho_{\overlap}: \dots q_{k} \xrightarrow{\epsilon} (q_k, p_0) \]
We then use the transitions in $(Q_1 \times Q_2)$ from position $i_2$ until $j_1$, here for every transition in the original runs within the interval we do one of two things:
\begin{itemize}
	\item If both $\cA_{\varphi_1}$ and $\cA_{\varphi_2}$ take a reading transition at position $\ell \in [i_2..j_1]$, the run $\rho_{\overlap}$ uses:
	$((q, p), P_1 \land P_2, L_1 \cup L_2, (q', p')) \in \Doverlap$.
	\item w.l.o.g. If $\cA_{\varphi_1}$ takes an $\epsilon$-transition, the run $\rho_{\overlap}$ mimics this step by doing an epsilon transition using $\{((q, p), \epsilon, (q', p)) \mid (q, \epsilon, q') \in \Delta_1\}$.
\end{itemize}
By following these rules, we reach the configuration in $\rho_{\overlap}$:
\[ \rho_{\overlap}: \dots ((q_{k}, p_0), i_2) \xrightarrow{t_{\overlap}} \dots \xrightarrow{t'_{\overlap}} ((q_n, p_{r+1}), j_1+1) \]
